...
namespace LXP.Core.Services
{
    public class LearnerProgressService : ILearnerProgressService
    {
        ...
        public async Task<double> LearnerProgress(ProgressViewModel learnerProgress)
        {
            ...
            if (
                !await _learnerProgressRepository.AnyLearnerProgressByLearnerIdAndMaterialId(
        ...
        {
            return await _learnerProgressRepository.GetLearnerProgressByLearnerIdAndMaterialId(
        ...
        public async Task<bool> Progress(LearnerProgressViewModel learnerProgress)
        {
            ...
            double totalCourseWatchtime = await CourseWatchTime(
        ...
        {
            var learnerProgress = await _learnerProgressRepository.GetLearnerProgressById(
                learnerId,
                courseId
            );
            double totalCourseWatchtime = await CourseWatchTime(
        ...
        {
            var learnermaterial = await _learnerProgressRepository.GetLearnerProgressByMaterialId(
                learnerId,
                materialId
            );
        ...
        public async Task<double> TopicWatchTime(Guid topicId, Guid learnerId)
        {
            ...
            foreach (var material in materials)
            {
                ...
                var materialdetail =
                    await _learnerProgressRepository.GetLearnerProgressByMaterialId(
                        learnerId,
                        materialId
                    );
