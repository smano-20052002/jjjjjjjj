...
public class CourseViewModelValidator : AbstractValidator<CourseViewModel>
{
    ...
    {
        RuleFor(course => course.Title).NotEmpty().WithMessage("Title is required");

        RuleFor(course => course.Level).NotEmpty().WithMessage("Level is required");

        RuleFor(course => course.Category).NotEmpty().WithMessage("Category is required");

        RuleFor(course => course.Description).NotEmpty().WithMessage("Description is required");
